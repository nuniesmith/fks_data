...
class BaseRepository[T: BaseModel, ID: (str, int, UUID)]:
    ...
    async def get_by_id(self, id: ID) -> Optional[T]:
        ...
        try:
            ...
            raise DatabaseError(
                message=f"Failed to get {self.model_cls.__name__} with ID {id}",
    ...
    async def find_one(self, filter: QueryFilter) -> Optional[T]:
        ...
        try:
            ...
            raise DatabaseQueryError(
                message=f"Failed to find {self.model_cls.__name__}",
                details={"error": str(e)},
            )
    ...
    async def find_all(
        ...
        try:
            ...
            raise DatabaseQueryError(
                message=f"Failed to find {self.model_cls.__name__} records",
                details={"error": str(e)},
            )
    ...
    async def create(self, data: Union[T, dict[str, Any]]) -> T:
        ...
        try:
            ...
            filtered_data = {
                k: v for k, v in data_dict.items() if k in self.model_fields
            }
            ...
            if isinstance(e, (ValueError, TypeError)):
                raise DataValidationException(
                    message=f"Invalid data for {self.model_cls.__name__}",
                    details={"error": str(e)},
                )
            else:
                raise DatabaseError(
                    message=f"Failed to create {self.model_cls.__name__}",
                    details={"error": str(e)},
                )
    ...
    async def update(self, id: ID, data: Union[T, dict[str, Any]]) -> T:
        ...
        try:
            ...
            filtered_data = {
                ...
                if k in self.model_fields and k != self.id_field
            }
    ...
    async def delete(self, id: ID) -> bool:
        ...
        try:
            ...
            raise DatabaseError(
                message=f"Failed to delete {self.model_cls.__name__} with ID {id}",
                details={"id": id, "error": str(e)},
            )
    ...
    async def count(self, filter: Optional[QueryFilter] = None) -> int:
        ...
        try:
            ...
            raise DatabaseQueryError(
                message=f"Failed to count {self.model_cls.__name__} records",
                details={"error": str(e)},
            )
    ...
    async def execute_raw(
        ...
        try:
            ...
            raise DatabaseQueryError(
                message="Failed to execute raw SQL query",
                details={"error": str(e)},
                sql=query,
            )
